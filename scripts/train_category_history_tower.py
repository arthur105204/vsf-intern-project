#!/usr/bin/env python3
"""Train the category-aware history two-tower retrieval model."""
...
from src.models.two_tower import (
    ...
    count_items_with_categories,
    count_users_with_non_empty_history,
...
def write_training_report(path: Path, summary: dict, history: list[float]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as f:
        f.write("# Category-Aware History Two-Tower Training Summary\n\n")
        ...
        f.write(f"- Users with non-empty train history: {summary['users_with_non_empty_history']}\n")
        f.write(f"- Model items in vocab: {summary['model_items_in_vocab']}\n")
        f.write(f"- Model vocab items with category: {summary['model_items_with_category']}\n")
        f.write(f"- Category vocab size: {summary['category_vocab_size']}\n")
...
def main():
    ...
    parser.add_argument("--out_dir", default="outputs/experiments/category_history_tower")
    parser.add_argument(
        "--item_properties_paths",
        nargs="+",
        default=["data/raw/item_properties_part1.csv", "data/raw/item_properties_part2.csv"],
    )
    parser.add_argument("--category_tree", default="data/raw/category_tree.csv")
    ...
    category_ids = category_tree_ids_from_csv(Path(args.category_tree))
    category_map = build_item_category_map(
        [Path(path) for path in args.item_properties_paths],
        item_ids=set(vocabs.item_to_idx.keys()),
        category_ids=category_ids,
        max_timestamp_ms=timestamp_to_epoch_ms(effective_train_df["timestamp"].max()),
    )
    category_to_idx = build_category_vocab(category_map)
    item_category_indices = build_item_category_index_tensor(vocabs, category_map, category_to_idx)

    train_summary = summarize_training_frame(effective_train_df)
    train_summary.update(
        {
            "users_with_non_empty_history": count_users_with_non_empty_history(effective_train_df),
            "model_items_in_vocab": len(vocabs.item_to_idx),
            "model_items_with_category": count_items_with_categories(vocabs, category_map),
    ...
    config = TwoTowerConfig(
        ...
        users_with_non_empty_history=train_summary["users_with_non_empty_history"],
        device=args.device,
    )

    loader = build_history_training_loader(
        effective_train_df,
        vocabs,
